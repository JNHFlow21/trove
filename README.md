...
Secret values resolve only through Agent Switch; environment variables enable
providers and select secret names but never carry values. Local embedding and
every Vault read work without it. Cloud providers and source Provider key
capture require Agent Switch (github.com/JNHFlow21/agent-switch).
